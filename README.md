...
| 패키지 | 역할 | 의존 방향 |
|--------|------|-----------|
| `adapter.in.web` | HTTP adapter. 요청/응답 DTO와 예외 응답 매핑 | `application` 호출 |
| `adapter.in.scheduler` | Recovery Job 스케줄 진입점 | `application` 호출 |
| `application` | 예약, 결제, 복구 유스케이스 | `domain`, `application.port.out` 사용 |
| `application.port.out` | Redis admission, 외부 PG 같은 outbound port | adapter 구현에 비의존 |
| `domain` | 엔티티, 리포지토리, 상태 enum | 외부 레이어 비의존 |
| `adapter.out.redis/payment` | Redis, PG simulator, Resilience4j adapter | application port 구현 |
